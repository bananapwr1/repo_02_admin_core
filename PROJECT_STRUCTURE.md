# 📁 Структура проекта

Подробное описание архитектуры Admin Panel Bot.

## 🗂 Обзор структуры

```
/workspace/
│
├── 📄 bot.py                      # Главный файл запуска бота
├── 📄 requirements.txt            # Список зависимостей Python
├── 📄 .env                        # Конфигурация (не в git!)
├── 📄 .env.example               # Пример конфигурации
├── 📄 .gitignore                 # Игнорируемые файлы для git
│
├── 📄 README.md                  # Основная документация
├── 📄 QUICKSTART.md              # Быстрый старт
├── 📄 DEPLOYMENT.md              # Инструкция по развертыванию
├── 📄 CHANGELOG.md               # История изменений
├── 📄 PROJECT_STRUCTURE.md       # Этот файл
│
├── 📄 supabase_schema.sql        # SQL-схема для базы данных
│
├── 📁 config/                    # Конфигурация приложения
│   ├── __init__.py
│   └── settings.py               # Настройки из .env
│
├── 📁 database/                  # Работа с Supabase
│   ├── __init__.py
│   └── supabase_connector.py     # Класс для операций с БД
│
├── 📁 handlers/                  # Обработчики команд
│   ├── __init__.py               # Регистрация роутеров
│   ├── start_handler.py          # /start, /help, /stats
│   ├── users_handler.py          # Управление пользователями
│   ├── strategies_handler.py     # Управление стратегиями
│   ├── tokens_handler.py         # Генерация токенов
│   ├── logs_handler.py           # Просмотр логов
│   └── settings_handler.py       # Настройки бота
│
├── 📁 keyboards/                 # Клавиатуры Telegram
│   ├── __init__.py
│   └── admin_keyboards.py        # Все inline и reply клавиатуры
│
├── 📁 middleware/                # Middleware для обработки
│   ├── __init__.py
│   └── admin_middleware.py       # Проверка прав админа
│
├── 📁 services/                  # Бизнес-логика
│   ├── __init__.py
│   ├── strategy_manager_service.py # Управление стратегиями + шифрование
│   └── trading_core_service.py     # Автономное ядро генерации сигналов
│
└── 📁 utils/                     # Утилиты
    ├── __init__.py
    ├── formatters.py             # Форматирование данных
    └── validators.py             # Валидация входных данных
```

## 📄 Основные файлы

### bot.py
**Главный файл приложения**

Функции:
- Инициализация бота и диспетчера
- Регистрация middleware и роутеров
- Обработчики startup/shutdown
- Запуск polling

```python
# Основная точка входа
if __name__ == "__main__":
    asyncio.run(main())
```

### requirements.txt
**Зависимости проекта**

```
aiogram==3.4.1          # Telegram Bot Framework
supabase==2.3.4         # Database Client
python-dotenv==1.0.0    # Environment Variables
pydantic==2.6.1         # Data Validation
aiohttp==3.9.3          # HTTP Client
```

### .env
**Конфигурационный файл (не в репозитории)**

Содержит:
- Telegram Bot Token
- Supabase credentials
- Список админов
- Настройки бота

## 📁 Модули

### 🔧 config/

#### settings.py
**Класс Settings** - Центральная конфигурация

```python
class Settings:
    TELEGRAM_BOT_TOKEN: str
    SUPABASE_URL: str
    SUPABASE_KEY: str
    ADMIN_IDS: list[int]
    BOT_NAME: str
    WELCOME_MESSAGE: str
```

Загружает переменные из .env и предоставляет их всему приложению.

### 🗄 database/

#### supabase_connector.py
**Класс SupabaseConnector** - Работа с БД

Методы:
- `get_all_users()` - Список пользователей
- `get_user_by_id()` - Пользователь по ID
- `update_user_status()` - Блокировка/разблокировка
- `update_user_subscription()` - Обновление подписки
- `create_invite_token()` - Создание токена
- `get_all_strategies()` - Список стратегий
- `create_strategy()` - Создание стратегии
- `get_system_logs()` - Системные логи
- `get_decision_logs()` - Логи решений Ядра (reasoning logs)
- и многое другое...

Singleton паттерн:
```python
db = SupabaseConnector()
```

### 🎮 handlers/

Каждый handler отвечает за свою область функциональности.

#### start_handler.py
- `/start` - Главное меню
- `/help` - Справка
- `/stats` - Быстрая статистика

#### users_handler.py
- Просмотр списка пользователей
- Поиск по ID
- Блокировка/разблокировка
- Изменение подписки
- Статистика по пользователю

#### strategies_handler.py
- Список стратегий
- Информация о стратегии
- Активация/деактивация
- Анализ производительности

#### tokens_handler.py
- Генерация токенов (одноразовые/многоразовые)
- Список токенов
- Деактивация токенов

#### logs_handler.py
- Системные логи
- Логи решений Ядра (reasoning logs)
- Общая статистика
- Детальная статистика

#### settings_handler.py
- Настройки бота
- Системная информация
- Общие callback'и (main_menu, noop)

### ⌨️ keyboards/

#### admin_keyboards.py
**Все клавиатуры бота**

Reply клавиатуры:
- `get_main_menu_keyboard()` - Главное меню

Inline клавиатуры:
- `get_users_menu_keyboard()` - Меню пользователей
- `get_user_action_keyboard()` - Действия с пользователем
- `get_subscription_types_keyboard()` - Типы подписок
- `get_strategies_menu_keyboard()` - Меню стратегий
- `get_tokens_menu_keyboard()` - Меню токенов
- `get_logs_menu_keyboard()` - Меню логов
- И другие...

### 🛡 middleware/

#### admin_middleware.py
**Класс AdminMiddleware** - Проверка прав

```python
class AdminMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data):
        # Проверка ID в ADMIN_IDS
        # Логирование действий
        # Блокировка неавторизованных
```

Применяется ко всем сообщениям и callback'ам.

### 🤖 services/

#### trading_core_service.py
**TradingLogicCore** - автономное ядро генерации сигналов и reasoning logs

Что делает:
- Читает активные стратегии из Supabase
- Проверяет условия индикаторов по шаблонам (TRUE/FALSE)
- Пишет reasoning logs в `decision_logs`
- При LONG/SHORT создаёт запись в `signals`

### 🔧 utils/

#### formatters.py
**Функции форматирования**

- `format_user_info()` - Информация о пользователе
- `format_strategy_info()` - Информация о стратегии
- `format_token_info()` - Информация о токене
- `format_log_entry()` - Запись лога
- `format_decision_log()` - Лог решения Ядра (reasoning log)
- `format_statistics()` - Общая статистика
- `format_datetime()` - Дата и время
- `format_json()` - JSON для отображения
- `paginate_list()` - Пагинация

#### validators.py
**Функции валидации**

- `validate_telegram_id()` - Проверка Telegram ID
- `validate_subscription_type()` - Проверка типа подписки
- `validate_token()` - Проверка формата токена
- `validate_strategy_name()` - Проверка имени стратегии
- `validate_assets_list()` - Проверка списка активов
- `validate_timeframe()` - Проверка таймфрейма
- `sanitize_input()` - Очистка ввода
- `is_valid_email()` - Проверка email

## 🔄 Поток данных

### 1. Пользователь отправляет команду
```
Telegram → bot.py → Dispatcher → AdminMiddleware → Handler
```

### 2. Handler обрабатывает запрос
```
Handler → Database/Service → Supabase → Handler
```

### 3. Ответ пользователю
```
Handler → Keyboard → Telegram
```

## 🗄 База данных

### Таблицы Supabase

1. **users** - Пользователи системы
2. **strategies** - Торговые стратегии
3. **invite_tokens** - Токены приглашения
4. **system_logs** - Системные логи
5. **decision_logs** - Логи решений Ядра (reasoning logs)
6. **signals** - Торговые сигналы
7. **trades** - Выполненные трейды
8. **bot_settings** - Настройки бота

Подробная схема в `supabase_schema.sql`

## 🔐 Безопасность

### Middleware
Проверяет ID пользователя перед каждым запросом

### Валидация
Все входные данные проходят валидацию

### Логирование
Все действия админов логируются

### Environment
Чувствительные данные только в .env

## 📊 Логирование

### Уровни
- `INFO` - Обычные события
- `WARNING` - Предупреждения
- `ERROR` - Ошибки

### Файлы
- Консоль (stdout)
- `admin_bot.log` (файл)

## 🧩 Расширение

### Добавление нового раздела

1. Создайте handler:
```python
# handlers/new_handler.py
from aiogram import Router
router = Router()

@router.message(F.text == "Новый раздел")
async def new_section(message: Message):
    await message.answer("Новая функция!")
```

2. Добавьте клавиатуру:
```python
# keyboards/admin_keyboards.py
def get_new_keyboard():
    return InlineKeyboardMarkup(...)
```

3. Зарегистрируйте:
```python
# handlers/__init__.py
from . import new_handler
main_router.include_router(new_handler.router)
```

### Добавление новой таблицы

1. Создайте в Supabase
2. Добавьте методы в `supabase_connector.py`
3. Используйте в handlers

## 📈 Производительность

### Асинхронность
Все операции I/O выполняются асинхронно

### Кэширование
Кэширование используется в сервисах агрегации/аналитики (TTL)

### Пагинация
Длинные списки разбиваются на страницы

## 🔄 Жизненный цикл

### Startup
1. Проверка конфигурации
2. Подключение к Supabase
3. Уведомление админов
4. Запуск polling

### Runtime
1. Получение update
2. Middleware проверка
3. Routing к handler
4. Обработка и ответ

### Shutdown
1. Остановка polling
2. Уведомление админов
3. Закрытие соединений

---

**Версия:** 1.0.0
**Дата:** 2025-12-11
