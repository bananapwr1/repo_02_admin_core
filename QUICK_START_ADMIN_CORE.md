# 🚀 Быстрый Старт - Admin Core

## 📋 Чеклист Установки

### ✅ Шаг 1: Установка Зависимостей

```bash
pip install -r requirements.txt
```

**Новая зависимость:** `cryptography==42.0.5`

---

### ✅ Шаг 2: Генерация Ключа Шифрования

**Вариант А: Используйте скрипт**
```bash
python generate_encryption_key.py
```

**Вариант Б: Через командную строку**
```bash
python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"
```

Скопируйте полученный ключ.

---

### ✅ Шаг 3: Настройка .env файла

Откройте `.env` и добавьте/обновите следующие переменные:

```env
# ============= ОСНОВНЫЕ НАСТРОЙКИ =============
TELEGRAM_BOT_TOKEN_ADMIN=your_telegram_bot_token_here

# ============= SUPABASE (ОБЯЗАТЕЛЬНО!) =============
SUPABASE_URL=https://your-project.supabase.co
SUPABASE_SERVICE_ROLE_KEY=your_service_role_key_200plus_chars

# ============= НОВЫЕ ПЕРЕМЕННЫЕ =============
# Chat ID администратора для системных уведомлений
# Узнать можно через @userinfobot или @getmyid_bot
ADMIN_CHAT_ID=123456789

# Ключ шифрования (сгенерированный на шаге 2)
ENCRYPTION_KEY=полученный_ключ_шифрования

# ============= ОПЦИОНАЛЬНЫЕ =============
OPENAI_API_KEY=your_openai_key
ADMIN_IDS=123456789,987654321
```

**Проверьте:**
- ✅ `TELEGRAM_BOT_TOKEN_ADMIN` - токен вашего админ-бота
- ✅ `SUPABASE_URL` - URL вашего Supabase проекта
- ✅ `SUPABASE_SERVICE_ROLE_KEY` - Service Role Key (200+ символов!)
- ✅ `ADMIN_CHAT_ID` - ваш Telegram ID
- ✅ `ENCRYPTION_KEY` - сгенерированный ключ

---

### ✅ Шаг 4: Обновление Базы Данных

Выполните миграцию в Supabase SQL Editor:

```bash
# Откройте файл
cat supabase_migration_encrypted_fields.sql

# Скопируйте содержимое и выполните в Supabase Dashboard
# https://supabase.com/dashboard -> SQL Editor
```

Или выполните SQL напрямую:

```sql
ALTER TABLE strategies 
ADD COLUMN IF NOT EXISTS api_keys_encrypted TEXT,
ADD COLUMN IF NOT EXISTS secret_keys_encrypted TEXT,
ADD COLUMN IF NOT EXISTS private_params_encrypted TEXT,
ADD COLUMN IF NOT EXISTS credentials_encrypted TEXT;
```

---

### ✅ Шаг 5: Тестирование

Запустите тестовый скрипт:

```bash
python test_admin_core_features.py
```

**Ожидаемый результат:**
```
✅ ПРОЙДЕН: Шифрование
✅ ПРОЙДЕН: Создание стратегии
✅ ПРОЙДЕН: Активация/деактивация
✅ ПРОЙДЕН: Уведомления

Итого: 4/4 тестов пройдено

🎉 ВСЕ ТЕСТЫ ПРОЙДЕНЫ УСПЕШНО!
```

---

### ✅ Шаг 6: Запуск Бота

```bash
python bot.py
```

**Ожидаемый вывод:**
```
🚀 Admin Core запускается...
✅ Конфигурация проверена
✅ Подключение к Supabase установлено (используется Service Role Key)
✅ Admin Core успешно запущен!
```

**Проверьте Telegram:**
Вы должны получить уведомление:
```
✅ Admin Core запущен!

🔐 Service Role Key активен
📊 Система готова к управлению стратегиями
```

---

## 🎯 Быстрая Проверка Функционала

### Тест 1: Создание Стратегии

```python
from services import get_strategy_manager
import asyncio

async def test():
    manager = get_strategy_manager()
    
    strategy_id = await manager.create_strategy(
        name="Test Strategy",
        description="Testing Admin Core",
        is_active=False,
        assets_to_monitor=["BTC/USDT"],
        api_keys={"test": "secret_key"}  # Будет зашифровано
    )
    
    print(f"✅ Стратегия создана: ID {strategy_id}")

asyncio.run(test())
```

### Тест 2: Проверка Шифрования

```python
from services.strategy_manager_service import EncryptionService

encryption = EncryptionService()

# Шифруем
encrypted = encryption.encrypt("my_secret_data")
print(f"Зашифровано: {encrypted[:30]}...")

# Расшифровываем
decrypted = encryption.decrypt(encrypted)
print(f"Расшифровано: {decrypted}")
```

### Тест 3: Отправка Уведомления

Просто запустите и остановите бота - вы получите уведомления автоматически.

---

## 🔍 Устранение Проблем

### Проблема 1: "ENCRYPTION_KEY не установлен"

**Решение:**
```bash
# Генерируйте ключ
python generate_encryption_key.py

# Добавьте в .env
ENCRYPTION_KEY=полученный_ключ
```

### Проблема 2: "ADMIN_CHAT_ID не установлен"

**Решение:**
```bash
# 1. Откройте @userinfobot в Telegram
# 2. Отправьте любое сообщение
# 3. Скопируйте ваш ID
# 4. Добавьте в .env
ADMIN_CHAT_ID=ваш_id
```

### Проблема 3: "Таблица strategies не существует"

**Решение:**
```bash
# Выполните основную схему БД
cat supabase_schema.sql
# Скопируйте и выполните в Supabase SQL Editor
```

### Проблема 4: "Invalid API key" от Supabase

**Решение:**
```bash
# Проверьте, что используете Service Role Key, а не Anon Key
# Service Role Key:
# - Длина 200+ символов
# - Начинается с "eyJ"
# - Находится в Supabase Dashboard -> Settings -> API -> service_role
```

### Проблема 5: Уведомления не приходят

**Проверьте:**
1. ✅ `ADMIN_CHAT_ID` установлен правильно
2. ✅ Бот добавлен в чат/разговор
3. ✅ Бот не заблокирован пользователем
4. ✅ `TELEGRAM_BOT_TOKEN_ADMIN` корректный

---

## 📚 Дополнительные Ресурсы

- **Полная документация:** `ADMIN_CORE_IMPLEMENTATION.md`
- **Примеры использования:** `EXAMPLES_USAGE.md`
- **Исходный код:**
  - `services/notification_service.py`
  - `services/strategy_manager_service.py`

---

## ✅ Чеклист Готовности

Перед запуском в продакшн проверьте:

- [ ] ✅ Все тесты пройдены (`python test_admin_core_features.py`)
- [ ] ✅ `ENCRYPTION_KEY` сгенерирован и установлен
- [ ] ✅ `ADMIN_CHAT_ID` настроен
- [ ] ✅ `SUPABASE_SERVICE_ROLE_KEY` проверен (200+ символов)
- [ ] ✅ Миграция БД выполнена
- [ ] ✅ Получено уведомление о запуске бота
- [ ] ✅ Создана и активирована тестовая стратегия
- [ ] ✅ Проверено шифрование конфиденциальных данных

---

## 🎉 Готово!

Admin Core настроен и готов к работе!

**Следующие шаги:**
1. Интегрируйте стратегии с Trading Core (Bot-1)
2. Создайте UI для управления стратегиями
3. Настройте мониторинг и логирование

---

**Дата:** 11 декабря 2025  
**Версия:** 1.0  
**Поддержка:** См. `ADMIN_CORE_IMPLEMENTATION.md`
